...
*Лекція 5 показала, що відбувається з обмеженнями цілісності в момент
...
`WHERE`, що й у `SELECT` з Лекції 2, з тими самими операторами
порівняння.
...
**Обмеження цілісності з Лекції 5 діють і тут.** Спроба `UPDATE`, яка
...
рекомендувала Лекція 4 для ветклініки), ця команда завершиться
...
- `DELETE` з таблиці, на яку посилається `FOREIGN KEY`, підпорядковане
  правилу `ON DELETE`, обраному при `CREATE TABLE` (Лекція 4):
